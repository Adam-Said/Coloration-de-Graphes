...
int stockMessage(struct paquet stockage[], int* ind, int desc, int numSousAnneau){
    if(*ind == MAX_STOCKAGE){
        printInColor("Problème de dépassement du stockage maximum du message", numSousAnneau);
        return -1;
    }
}
